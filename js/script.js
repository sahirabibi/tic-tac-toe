...
function displayMove(target, currentPlayer) {
    // take the target and updates its inner HTML to display the move made by the player only if space is avialable
    if (target.innerText === "") {
...
function switchPlayers() {
    console.log("Hello from switchPlayer");
    if (currentPlayer === "X") {
        currentPlayer = "O";
...
// get board and add event listner to every sqr
// if sqr is clicked
...
board.addEventListener("click", (e) => {
    if (e.target.classList.contains("sqr")) {
        ...
        let target = e.target;
        displayMove(target, currentPlayer);
...
// Allow player to click on board to make move
    ...
    // remove eventlistner from that space so no other changes can occur to it.
